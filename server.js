    ...
    try {
        ...
        if (data.event === 'page_view') {
            ...
            if (!analytics.dailyStats[today]) {
                analytics.dailyStats[today] = { visitors: new Set(), pageviews: 0 };
            }
            analytics.dailyStats[today].visitors.add(event.visitor_hash);
            analytics.dailyStats[today].pageviews++;

            // Convert Set to Array for JSON storage
            analytics.dailyStats[today].visitors = Array.from(analytics.dailyStats[today].visitors);
        }
...
function handleAnalyticsGet(req, res, query) {
    try {
        ...
        const recent = pageViews.filter(e =>
            Date.now() - e.timestamp < days * 24 * 60 * 60 * 1000
        );
